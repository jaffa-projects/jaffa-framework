...
var gridSortBrowserVar = function() {
...
function doClick(e) {
  if(!gridSortBrowserVar.ie5) {
    doClickN(e)
  } else {
    if (e.srcElement.parentNode.parentNode.tagName == "THEAD" &&
        e.srcElement.parentNode.parentNode.getAttribute("sorting") != 'false') {
      var tbody= null;
      var colCount= null;
      if (!initialised)
        init(e);
...
function doClickN(e) {
  if(e.target.parentNode.parentNode.tagName == "THEAD" &&
     e.target.parentNode.parentNode.getAttribute("sorting") != 'false') {
    var tbody= null;
    var colCount= null;
    if (!initialised)
      initN(e);
...
function init(e) {
  ...
  for (var i=0; i<colCount; i++) {
    ...
    // remove the following to allow width to be maitained by configuration
...
function sortColumn(e) {
  ...
  if (gridSortBrowserVar.ie5)
    tmp = e.srcElement;
  else if (gridSortBrowserVar.dom)
    tmp = e.target;
...
function getInnerText(el) {
  if (gridSortBrowserVar.ie5) return el.innerText;    //Not needed but it is faster
  var str = "";
