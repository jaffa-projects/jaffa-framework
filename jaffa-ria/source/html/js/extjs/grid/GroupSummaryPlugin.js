 ...
 *
 * Modified by Sean Zhou @ MIRO Technologies, Inc.
 * Source: ext-3.4.0/examples/ux/GroupSummary.js
 *
 */

/**
 * @class Ext.grid.GroupSummaryPlugin
 * @extends Ext.util.Observable
 * A GridPanel plugin that enables dynamic column calculations and a dynamically
 * updated grouped summary row.
 */
Ext.grid.GroupSummaryPlugin = Ext.extend(Ext.util.Observable, {
    /**
...
summaryRenderer: function(v, params, data){
    ...
    constructor : function(config){
        ...
        Ext.grid.GroupSummaryPlugin.superclass.constructor.call(this);
    },
    init : function(grid){
        this.grid = grid;
    ...
    calculate : function(rs, cs){
        ...
        for(var j = 0, jlen = rs.length; j < jlen; j++){
            ...
            for(var i = 0, len = cs.length; i < len; i++){
                ...
                cf = cfg[i];
                if(cf.summaryType){
                    data[c.name] = Ext.grid.GroupSummaryPlugin.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data, cf);
                }
            }
        }
        return data;
    ...
    doGroupEnd : function(buf, g, cs, ds, colCount){
        ...
        buf.push('</div>', this.renderSummary({data: data}, cs, g.group), '</div>');
    },
    ...
    isGrouped : function(){
        var gf = this.grid.getStore().groupField;
        if (typeof gf == 'boolean') return gf;
        else if (typeof gf == 'undefined') return false;
        else if (Ext.isArray(gf)) return gf.length>0;
        else return !Ext.isEmpty(gf);
    },
    ...
    refreshSummaryById : function(gid){
        ...
        var cs = this.view.getColumnData(),
            ...
            markup = this.renderSummary({data: data}, cs, g.group),
...
Ext.grid.GroupSummaryPlugin.Calculations['totalCost'] = function(v, record, field){
...
Ext.grid.GroupSummaryPlugin.Calculations = {
    ...
    'countBoolean' : function(v, record, field, data){
      if (! data[field+'count']) {
        data[field+'count'] = 0;
      }
      return record.get(field) ? ++data[field+'count'] : data[field+'count'];
    },
    ...
    'label' : function(v, record, field, data, colConfig) {
      return colConfig.summaryLabel;
    }
};

/**
 * @class Ext.grid.HybridSummary
 * @extends Ext.grid.GroupSummaryPlugin
 ...
 *
 */
Ext.grid.HybridSummary = Ext.extend(Ext.grid.GroupSummaryPlugin, {
    /**
     * @private
     * @param {Object} rs
     * @param {Object} cs
...
Ext.preg('groupsummary', Ext.grid.GroupSummaryPlugin);
Ext.preg('hybridsummary', Ext.grid.HybridSummary);
