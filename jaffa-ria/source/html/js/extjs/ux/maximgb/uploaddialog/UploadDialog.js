...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button,
{
   ...
   * @access private
   */
  createInputFile : function(onRender)
  {
    ...
    this.adjustInputFileBox(onRender);
   ...
   * @access private
   */
  adjustInputFileBox : function(onRender)
  {
    ...
    if (this.el && this.input_file) {
      ...
      adj = {x: 3, y: 3}
      if (Ext.isIE) {
        // For IE, adjustments required during initial rendering and during subsequent creation of inputFileBox are different
        if (!onRender)
          adj = {x: -3, y: 3};

        this.input_file.setHeight(btn_box.height + 10);
      }
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  updateToolbar : function()
  {
    ...
    if (this.is_uploading) {
      ...
      if (tb.x_buttons.upload.rendered) {
        tb.x_buttons.upload.getEl()
          .child(tb.x_buttons.upload.buttonSelector)
