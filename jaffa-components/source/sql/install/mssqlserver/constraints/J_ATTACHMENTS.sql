...
IF EXISTS (SELECT 1 
             FROM sysobjects 
            WHERE xtype='PK' AND name='J_ATTACHMENTSP1'
           ) 
   BEGIN
        SELECT 'Constraint already exists, therefore not creating constraint: J_ATTACHMENTSP1.' 
   END
ELSE 
   BEGIN
        SELECT 'Constraint does not exist, therefore creating constraint: J_ATTACHMENTSP1.'
