...
public class JDBCLogger {
			...
			if (sql != null || sqlHandler != null) {
				...
				for (int i = 0; i < num; i++) {
					...
					if (logcol.logtype == JDBCLogType.MSG) {
						...
						// available
						String parameter = this.getThrowableRepresentationFromLoggingEvent(event);
						if (usePrepStmts || useCallStmts) {
							if (parameter == null) {
								prepStmt.setNull(indexToUse, logcol.sqlType);
							} else {
	...
	public void appendSQL(LoggingEvent aEvent, Layout layout) throws Exception {
		...
		try {
			...
			if (sqlHandler == null) {
				...
				new_sql = replace(new_sql, THROWABLE_WILDCARD, this.getThrowableRepresentationFromLoggingEvent(aEvent));
	...
	/**
	 * @param c Layout Parts Delimiter
	 */
