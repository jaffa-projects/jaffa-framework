 ...
 */
public class AttachmentViewerAction extends ActionBase {

    private static final Logger log = Logger.getLogger(AttachmentViewerAction.class);
    ...
    public FormKey do_Close_Clicked() {
        ...
        return ((AttachmentViewerForm) form).getComponent().quitAndReturnToCallingScreen();
    }
    ...
    public FormKey do_ViewAttachmentData_Clicked(){
        AttachmentViewerForm myForm = (AttachmentViewerForm) form;
        AttachmentViewerComponent myComp = (AttachmentViewerComponent) myForm.getComponent();
        FormKey fk = AttachmentComponentHelper.loadAttachmentData(myForm, request, myForm.getOriginalFileName(), myForm.getData());
        if (fk == null)
            fk = myComp.getViewerFormKey();
        return fk;
    }
